...
| deterministic byte transport harness | ready-for-review | byte frame encode/decode, fragmentation/reassembly, bounded byte pipe, sequence checks, corruption rejection, byte transport mutants, and generated parity | still uses deterministic in-memory byte pipe only | expand byte-path hardening and generated parity next |
