...
func TestGenerateFixtureSetCoversPrototype(t *testing.T) {
	...
	if set.BackendVersion != BackendVersion || set.Summary.StreamsOpened < 8 || set.Summary.BackpressureEvents == 0 || set.Summary.StreamsReset == 0 {
		t.Fatalf("prototype summary incomplete: %+v", set.Summary)
	}
	if set.PayloadLogged || set.SecretLogged {
		t.Fatalf("unsafe fixture logging flags set")
	}
}

func TestValidateConfigRejectsUnsafeModes(t *testing.T) {
	mutators := []func(*Config){
		func(c *Config) { c.AllowDNSResolution = true },
		func(c *Config) { c.AllowPublicNetworkDefault = true },
		func(c *Config) { c.AllowPacketCapture = true },
...
func TestScanForLeakRejectsUnsafeFields(t *testing.T) {
	unsafeCases := []any{
		...
		map[string]bool{"allow_dns_resolution": true},
	}
...
func TestStreamRunControlsRejected(t *testing.T) {
	set, err := GenerateFixtureSet()
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, run := range set.Runs {
		if run.Rejected {
			rejected++
			if run.RejectReasonBucket == "" || run.OpenResult != "rejected_before_stream_open" {
				t.Fatalf("rejected control missing safe reason: %+v", run)
			}
		}
	}
	if rejected < 3 {
		t.Fatalf("expected control stream rejections, got %d", rejected)
	}
}

func FuzzScanForLeak(f *testing.F) {
	f.Add(`{"content_class":"small_stream_marker"}`)
