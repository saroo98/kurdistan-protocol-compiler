...
// Historical constants describe immutable subjects, not this helper.
...
var helperOwnerV2PostHashesV1 = []string{"7707d4faf66e9d20edbb157a3ad59d71c81d8d3b7f869d7529ff312f9fce073d", "abf9e52b55971aefb21dace2226dfe4b29c4b5b8478504f30868934af8d6b935", "aa0d56ec1b1ebeeab11c90497d1f252295682bfb4b9d0c096dcd5b0047558ac0"}
...
func validateEvidenceOverlaysV1(root string, manifest committedEvidenceManifestV1) (map[string]string, error) {
	...
	for i, entry := range validators.Entries {
		if entry.Path != validatorPathsV1[i] || entry.PreSHA256 != validatorPreHashesV1[i] || !validHelperOwnerSHA256V1(entry.PostSHA256) || entry.PostSHA256 == entry.PreSHA256 || currentAtPre[entry.Path] != entry.PostSHA256 {
	...
	if consumerEntry.Path != "internal/testkit/importrules/importrules_test.go" || consumerEntry.PreSHA256 != "3a170c4752fea63a728d55abff9b0c8a7c91e25e0c98d14bdd4c401e3b56a178" || !validHelperOwnerSHA256V1(consumerEntry.PostSHA256) || consumerEntry.PostSHA256 == consumerEntry.PreSHA256 || currentAtPre[consumerEntry.Path] != consumerEntry.PostSHA256 {
	...
	for i, entry := range maintenance.Entries {
		if entry.Path != maintenancePathsV1[i] {
			return nil, fmt.Errorf("invalid maintenance entry %d", i)
		}
		if entry.PreSHA256 != maintenancePreHashesV1[i] || entry.PostSHA256 != maintenancePostHashesV1[i] {
			return nil, fmt.Errorf("maintenance hash drift at entry %d", i)
		}
	...
	for i, path := range helperOwnerPathsV1 {
		...
		if newEntry.Path != path || newEntry.PreSHA256 != oldEntry.PostSHA256 || newEntry.PostSHA256 != helperOwnerV2PostHashesV1[i] || !validHelperOwnerSHA256V1(newEntry.PostSHA256) || newEntry.PostSHA256 == newEntry.PreSHA256 {
