...
func ScanForLeak(value any) error {
	...
	if len(findings) > 0 {
		...
		return fmt.Errorf("%w: %s", ErrUnsafeMetadata, strings.Join(uniqueStrings(findings), ","))
	}
	return nil
}
...
func forbiddenKey(key string) bool {
	for _, marker := range forbiddenMarkers {
		if key == marker || strings.Contains(key, marker) {
			switch key {
			case "payload_logged", "secret_logged", "metadata_risk_bucket", "candidate_id", "new_candidate_id", "old_candidate_id", "active_path_id", "relay_risk_bucket", "relay_burn_detected", "relay_burn_signals", "relay_burn_blocked":
				return false
			}
			return true
		}
