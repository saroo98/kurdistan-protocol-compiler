...
func ValidateFixtureSet(set RelayBridgeFixtureSet) error {
	if set.Version != Version {
		return safeError("unknown_schema")
	}
	if len(set.Scenarios) < 12 || len(set.Sessions) == 0 || len(set.Streams) == 0 || len(set.Reports) == 0 {
	...
	if !MappingPreserved(set.Sessions, set.Streams) || IsolationViolations(set.Streams) != 0 {
...
func ScanMisuse(value any) RelayBridgeMisuseReport {
	report := RelayBridgeMisuseReport{Version: Version, ObjectsScanned: 1, Conclusion: "passed"}
...
func scanValue(value any, path string) error {
	...
	case map[string]any:
		for key, val := range typed {
			...
			if normalized == "payload_logged" || normalized == "secret_logged" {
				...
				return fmt.Errorf("forbidden relaybridge field %s", key)
			}
	...
	case string:
		...
		if isForbiddenToken(normalized) {
			return fmt.Errorf("forbidden relaybridge value in %s", path)
		}
	}
	return nil
}
...
func isForbiddenToken(value string) bool {
	for _, token := range []string{
		...
		"private_key", "session_secret", "real_relay", "dial_real",
	} {
