...
func TestProtectPrivatePathPreservesOwnerWithoutOwnerReplacement(t *testing.T) {
	...
	restrictWindowsPathWithoutWriteOwnerForTest(t, path, ownerBefore)
...
func TestProtectPrivatePathOwnerValidationRejectsUnexpectedOwner(t *testing.T) {
	...
	assertPrivateFileContent(t, path, []byte("previous"))
}
...
func TestProtectPrivatePathDACLFailuresRemainFailClosed(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			assertPrivateFileContent(t, path, want)
...
func TestProtectPrivateHandleOwnerRejectsDifferentSID(t *testing.T) {
	...
	handle := openPrivatePathHandleForTest(t, path, false, windows.FILE_READ_ATTRIBUTES|windows.READ_CONTROL)
...
func TestOwnerAndDACLPublicationRequiresWriteOwnerInNonprivilegedFixture(t *testing.T) {
	...
	restrictWindowsPathWithoutWriteOwnerForTest(t, path, owner)
...
func restrictWindowsPathWithoutWriteOwnerForTest(t *testing.T, path string, owner *windows.SID) {
	t.Helper()
...
func openPrivatePathHandleForTest(t *testing.T, path string, directory bool, access uint32) windows.Handle {
	t.Helper()
	pointer, err := windows.UTF16PtrFromString(path)
	if err != nil {
		t.Fatal(err)
...
func assertPrivateFileContent(t *testing.T, path string, want []byte) {
	t.Helper()
...
func TestProtectPrivatePathCanReplaceInheritedDACLWithoutOwnerRewrite(t *testing.T) {
	...
	restrictWindowsPathWithoutWriteOwnerForTest(t, path, windowsPathOwnerForTest(t, path))
	if err := openWindowsPathAccessForTest(path, windows.WRITE_DAC); err != nil {
		t.Fatalf("fixture lacks WRITE_DAC: %v", err)
	}
	if err := openWindowsPathAccessForTest(path, windows.WRITE_OWNER); !errors.Is(err, windows.ERROR_ACCESS_DENIED) {
...
func TestProtectPrivatePathRejectsInvalidSubjectsBeforeDACLMutation(t *testing.T) {
	root := t.TempDir()
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			if err := Protect(test.path, test.directory); !errors.Is(err, ErrUnsafe) {
				t.Fatalf("error=%v, want unsafe-filesystem rejection", err)
			}
			if err := Verify(test.path, test.directory); !errors.Is(err, ErrUnsafe) {
				t.Fatalf("Verify error=%v, want unsafe-filesystem rejection", err)
			}
		})
	}

	setCalls, closeCalls := 0, 0
	err := protectPrivatePathWithOperations(file, false, privatePathProtectionOperations{
		verifyOwner: func(windows.Handle, *windows.SID, *windows.SID) error { return ErrUnsafe },
	...
	if !errors.Is(err, ErrUnsafe) || setCalls != 0 || closeCalls != 1 {
