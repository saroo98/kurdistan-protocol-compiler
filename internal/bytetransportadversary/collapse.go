...
func AnalyzeRuns(runs []ScenarioRun, thresholds CollapseThresholds) Report {
	...
	for _, run := range runs {
		...
		frameKinds = append(frameKinds, "data")
		if run.Correct {
			report.Correctness.CorrectRuns++
		}
		if !run.Checks.RuntimeIntegrationCorrect {
			report.Correctness.RuntimeIntegrationFailures++
		}
		...
		if !run.Checks.CorruptionCorrect {
			report.Correctness.CorruptionFailures++
		}
		if !run.Checks.MalformedCorrect {
			report.Correctness.MalformedFailures++
		}
		if !run.Checks.ReassemblyCorrect {
			report.Correctness.ReassemblyFailures++
		}
	...
	report.FrameKinds = uniqueSorted(frameKinds)
	...
	if report.Correctness.RuntimeIntegrationFailures+report.Correctness.BackpressureFailures+report.Correctness.SequenceFailures+report.Correctness.CorruptionFailures+report.Correctness.MalformedFailures+report.Correctness.ReassemblyFailures+report.Correctness.ErrorResetFailures+report.Correctness.TraceHygieneFailures > 0 {
		report.Conclusion = "failed"
	}
...
func CollapseScenario(scenario string, runs []ScenarioRun, thresholds CollapseThresholds) ByteTransportCollapseReport {
	...
	if len(vectors) > 2 && score < thresholds.MinDiversityScore {
		suspicious = append(suspicious, "byte_transport_behavior_fixed")
	}
	if len(vectors) > 2 && dominantRatio(vectors, "policy_shape") > thresholds.MaxDominantRatio {
		suspicious = append(suspicious, "byte_policy_shape")
	}
	conclusion := "passed"
	if len(suspicious) > 0 {
		conclusion = "failed"
	}
	return ByteTransportCollapseReport{
		Scenario:          scenario,
		ProfileCount:      len(runs),
		FrameKinds:        []string{"data", "control"},
